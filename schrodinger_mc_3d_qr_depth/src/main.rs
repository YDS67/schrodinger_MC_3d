...
fn potential(x: f64, y: f64, z: f64, a: f64) -> f64 {
    if z < 2.0*QW_HEIGHT && z > QW_HEIGHT {
        U0*(1.0 - quantum_ring_shape(x, y)/a)
    } else {
        U1
    }
}

fn potential_plot(x: f64, y: f64, z: f64, a: f64) -> f64 {
    if z < 2.0*QW_HEIGHT && z > QW_HEIGHT {
        U0*(1.0 - quantum_ring_shape(x, y)/a)
    } else {
        1.5*U0
...
fn main() {
    ...
    for jx in 0..NX {
        for jz in 0..NZ {
            un2d[index_xz(jx,jz)] = - potential_plot(xn[jx], Y1, LZ-zn[jz], amplitude)
        }
    }
    draw::plot_2d(&un2d, NX, NZ, 2000, (2000*NZ)/NX, "Potential_shape_xz.png", "Turbo");
